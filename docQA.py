...
qdrant_client = QdrantClient(
    url="https://b4d4073f-b7a3-4d5d-a858-adad5cd46643.us-east4-0.gcp.cloud.qdrant.io:6333", 
    api_key="KiEiRaOWguHcKfFQOYvocM2r1PxNPJgZ1nsh_UPIsy2vJDD_uKb4Vg",
)
...
def ensure_table_exists():
    """Ensure the file_hashes table exists in the database."""
...
def is_duplicate_file(conn, file_hash):
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT 1 FROM file_hashes WHERE file_hash = %s", (file_hash,))
...
def add_file_to_db(conn, file_name, file_hash):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO file_hashes (file_name, file_hash) VALUES (%s, %s)",
                (file_name, file_hash)
            )
...
def add_success_status_to_db(conn, file_name):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE file_hashes SET added_to_vectorstore = TRUE WHERE file_name = %s",
                (file_name,)
            )
        conn.commit()  # Ensure the transaction is committed
...
def initialize_vectorstore(uploaded_files):
    ...
    # Ensure the file_hashes table exists
    ensure_table_exists()
